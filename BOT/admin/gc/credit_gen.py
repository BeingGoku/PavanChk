...
Contact the bot owner @lord_hanumant_bot!
</b>"""
...
async def generate_redeem_codes(client, message):
    try:
        ...
        for _ in range(amount):
            redeem_code = f"HANUMAN-{gcgenfunc()}{gcgenfunc()}{gcgenfunc()}-CHK"
            await insert_pm(redeem_code)
            response_text += f"➔ <code>{redeem_code}</code>\n"

        response_text += """<b>\nYou can redeem this code using this command: /redeem HANUMAN-XXXX-CHK</b>"""
