...
Message: We Didn't Find Any CC In @{channel_link}.
            ...
            file_name = f"downloads/{amt}Scrapped_{message.from_user.id}_@HanumanChkBot.txt"
...
● Bot by - <a href="https://t.me/lord_hanumant_bot">Hanuman</a>
"""
...
Message: Scrapping {limit} CC From {channel_title}.
...
Message: We Didn't Find Any CC In {channel_title}.
            ...
            file_name = f"downloads/{amt}Scrapped_{message.from_user.id}_@HitlerChkBot.txt"
            with open(file_name, "a", encoding="UTF-8") as f:
                for x in ccs:
                    cc, mes, ano, cvv = x[0], x[1], x[2], x[3]
                    fullcc = f"{cc}|{mes}|{ano}|{cvv}"
...
● Scraped By: <a href="tg://user?id={message.from_user.id}"> {message.from_user.first_name}</a> ♻️ [ {role} ]
● Time Taken: {min} Minutes {sec} Seconds
● Bot by - <a href="tg://user?id=7113416108">Goku </a>
</b>
"""
            await message.reply_document(document=file_name, caption=caption, reply_to_message_id=message.id)
            os.remove(file_name)
...
Message: Scrapping {limit} CC From {channel_title} . Please Wait . 
...
Message: Scrapping {limit} SK From {channel_title} . Please Wait . 
